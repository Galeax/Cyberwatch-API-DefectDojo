...
def get_assets_of_group(group_id):
    """
    Retrieve the list of assets (servers) for a given group ID.
    """
    all_assets = []
    page = 1
    while True:
    ...
    # Optionally, save assets to a file if needed:
    with open("assets.json", "w", encoding="utf-8") as file_out:
        file_out.write(json.dumps(all_assets, indent=4))
    return all_assets
...
def get_cwe_db():
    """
    Retrieve the cwe database from https://raw.githubusercontent.com/Galeax/CVE2CAPEC/refs/heads/main/resources/cwe_db.json
    """
    try:
        response = requests.get("https://raw.githubusercontent.com/Galeax/CVE2CAPEC/refs/heads/main/resources/cwe_db.json")
        cwe_db = response.json()
        return cwe_db
    except requests.exceptions.RequestException as exc:
        logging.error("Error while getting cwe database: %s", exc)
        return None
    
def get_cwe_parents_list(cwe_db, cwe_id):
    """
    Retrieve the list of parent CWEs for a given CWE ID.
    """
    parents = set()
    current_cwes = [cwe_id]
    ...
    while current_cwes:
        next_cwes = []
        for current_cwe in current_cwes:
            if current_cwe in cwe_db and "ChildOf" in cwe_db[current_cwe]:
                parent_cwes = cwe_db[current_cwe]["ChildOf"]
                parents.update(parent_cwes)
                next_cwes.extend(parent_cwes)
        current_cwes = next_cwes

    return list(parents)

def forge_cve_json(cve):
    """
    Forge a single CVE JSON object (instead of a line) from the API data.
    """
    # Initialize the base cve_json structure
    cve_json = {
        "type": "CVE",
    ...
    # Retrieve the CWE parents using the get_cwe_parents_list function
    if cve.get('cwe') and cve['cwe'].get('cwe_id'):
    ...
    for server in cve['servers']:
        ...
        cve_json["servers"].append(server_info)
    ...
    cve_json["updates_assets"] = updates_assets

    return cve_json


def get_all_cves_details(group):
    """
    Retrieve all CVEs for the given group, fetch their details,
    and return a list of proper JSON objects.
    """
    ...
    for cve in cves:
        try:
            ...
            response_data = next(response).json()
            cve_json = forge_cve_json(response_data)
            cves_details.append(cve_json)
        except requests.exceptions.RequestException as exc:
...
def forge_security_issue_json(security_issue):
    """
    Forge a single Security Issue JSON object from the API data,
    ...
    for server in relevant_servers:
        ...
        security_issue_json["servers"].append(server_info)
    return security_issue_json
...
def get_all_security_issues_details():
    """
    Retrieve all security issues, fetch their details,
    and return a list of proper JSON objects.
    """
    ...
    for security_issue in security_issues:
        try:
            ...
            response_data = next(response).json()
            issue_json = forge_security_issue_json(response_data)
            if issue_json:
                security_issues_details.append(issue_json)
        except requests.exceptions.RequestException as exc:
...
def main():
    ...
    retrieves assets, CVEs, and Security Issues, then writes them as a proper JSON file.
    """
    ...
    # Get CVE and Security Issue details as lists of dictionaries
    cves_details = get_all_cves_details(group)
    security_issues_details = get_all_security_issues_details()

    # Combine the results into one final structure.
    output = {
        "cves": cves_details,
        "security_issues": security_issues_details
    }
